...
3. Pro PR (in Reihenfolge, max. 3 Aktionen pro Lauf):
   ...
   `sessions_spawn({ agentId: "feature-dev-reviewer", model: "deepseek-flash", label: "review-<n>", task: "Kritischer Review von PR #<n> in momokli/openclaw-deploy (Diff, Tests, Security, Doku, AGENTS.md-Checkliste). Als PR-Kommentar posten, ERSTE Zeile exakt `[VERDICT: APPROVE]`oder`[VERDICT: REQUEST_CHANGES]` + bei REQUEST_CHANGES die konkreten Blocker als Liste. KEIN Merge." })`
