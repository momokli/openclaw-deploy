...
3. Pro PR (in Reihenfolge, **ein** PR pro Lauf aktiv bearbeiten, max. 3 Aktionen pro Lauf):
   ...
   `sessions_spawn({ agentId: "feature-dev-reviewer", model: "openrouter/deepseek/deepseek-v4.1-flash", label: "review-<n>", task: "Kritischer Review von PR #<n> in momokli/riftbreaker-battle-mod (Diff, Tests, Security, Doku, AGENTS.md-Checkliste). Als PR-Kommentar posten, ERSTE Zeile exakt `[VERDICT: APPROVE]`oder`[VERDICT: REQUEST_CHANGES]` + bei REQUEST_CHANGES die konkreten Blocker als Liste. KEIN Merge." })`
